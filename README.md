...
1. What is the DOM?
   ...
   It's like a tree. You have elements which are inside other elements that are inside even more elements. You have branches on this tree that are objects which have their own properties we can change, or add.

2. What is an event?
   Something that can be triggered. Mouseclick is an example.

3. What is an event listener?
   The event listener is added to an event. When the event is invoked the listener fires logic.
4. Why would we convert a NodeList into an Array?
   Because when using querySelectorAll() it returns that NodeList. We can only use forEach on that. Actually using something like Array.from(variable) will allow us to use more array methods ... filter() map() etc

5. What is a component?
   Reusable blocks of code. They're isolated in their own files, and return HTML.
